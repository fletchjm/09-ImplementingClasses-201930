...
class Point(object):
    ...
    def move_by(self, dx, dy):
        self.travel = self. travel +self.get_distance_from(Point(self.x+dx,
                                                           self.y+dy))
    ...
    def get_distance_from(self, another):
        distance =((self.x - another.x) ** 2 + (self.y - another.y) ** 2)** .5
        return distance

    def get_distance_from_start(self):
        distancex = (self.x - self.startx) ** 2
        distancey = (self.y - self.starty) ** 2
        distance = (distancex + distancey) ** .5
        return distance
    ...
    def closer_to(self, p2, p3):
        distp2 = self.get_distance_from(p2)
        distp3 = self.get_distance_from(p3)
